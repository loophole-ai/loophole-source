...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|loophole-ai|loophole-insiders|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun
  else
  ...
  # workaround that enforces x86 ARCH for pkg2appimage having /__w/loophole-ai/loophole-ide/build/linux/appimage/Loophole/Loophole.AppDir/usr/share/loophole/resources/app/node_modules/rc/index.js is of architecture armhf
  export ARCH=x86_64
